...
if (httpOk) {
  if (!body || typeof body !== "object") {
    ...
    for (const lane of lanes) {
      ...
      const ok =
        lane.available === true &&
        lane.freshnessState === "fresh" &&
        !usingLastKnownGood &&
        !failedLaneIds.includes(laneId);
      laneSummary[laneId] = {
      ...
      if (!ok) {
        failedChecks.push(
          "Lane " +
            laneId +
            " not fresh/complete for current cycle (available=" +
            lane.available +
            ", freshnessState=" +
            lane.freshnessState +
            ", usingLastKnownGood=" +
...
const freshLaneCount = Object.values(laneSummary).filter((l) => l.ok).length;
const staleLaneCount = Object.values(laneSummary).filter((l) => !l.ok).length;

const healthSummary = {
  total_lanes: lanes.length,
  fresh_lane_count: freshLaneCount,
  stale_or_incomplete_lane_count: staleLaneCount,
  failed_lane_count: failedLaneIds.length,
  degraded_lane_count: degradedLaneIds.length,
...
const result = {
  ...
  degraded_lane_ids: degradedLaneIds,
  note:
    "Watchdog checks system-health.json (the real published document) as the freshness/completion proxy. monitor-status.json referenced in the original task spec does not exist at the UWISSY public site as of 2026-08-20; required-lane list is the lane set present in the document, not an externally defined list. degradedLaneIds is reported for visibility but is not itself a hard failure unless it also carries freshnessState!=fresh, usingLastKnownGood=true, or a failedLaneIds entry, consistent with the project rule that degraded/partial source data is not the same as pipeline failure."
};
