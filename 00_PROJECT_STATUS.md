...
- **Public presentation policy:** `scripts/build-public-package-snapshot.mjs` now judges every candidate event for route relevance, route-use effect, event type (health alerts excluded), flood severity (major-or-worse only), government CAP severity (Severe/Extreme only), freshness, and duplicate status before it may reach `route-events.geojson`. Ineligible events are never deleted — each is written with a machine-readable reason to `data/connectors/audit/exclusions-<releaseId>.json` (tracked, never served). `src/lib/route-status/presentation-eligibility.ts` is a dashboard-side final guard, independent of the data layer.
